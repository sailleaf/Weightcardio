...
public class AlterAge {
    ...
    public AlertDialog.Builder getDialog(final Activity activity) {
        ...
        builder.setTitle("Update Age");             //设置标题
        builder.setIcon(R.mipmap.ic_launcher);//设置图标，图片id即可

        //  载入布局
        LayoutInflater inflater = activity.getLayoutInflater();
        View layout = inflater.inflate(R.layout.dialog_alert_age, null);
        builder.setView(layout);

        //  对布局中的控件监听
        etAlterAge = (EditText) layout.findViewById(R.id.input_alter_age);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    String param = "userid=" + lastDeviceId + account + "&age="
                            + etAlterAge.getText().toString();

                    Log.e(TAG, "" + param);
                    String rs = HttpRequestUtil.sendGet(AppConstants.URL_UPDATE_SIGN_UP_INFO, param);
                    Log.e(TAG, "" + rs);
                    mUIHandler.obtainMessage(SUCCESS_COMPLETE, "Alter Age Successfully").sendToTarget();
                } catch (Exception ex) {
