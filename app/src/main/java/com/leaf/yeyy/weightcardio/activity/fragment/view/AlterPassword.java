...
public class AlterPassword {
    ...
    private static final String TAG = AlterPassword.class.getSimpleName();
    ...
    public AlertDialog.Builder getDialog(final Activity activity) {
        ...
        //  对布局中的控件监听
        etAlterPassword = (EditText) layout.findViewById(R.id.input_alter_password);
        etAlterPasswordConfirm = (EditText) layout.findViewById(R.id.input_alter_confirm);
    ...
    private void updateRegisterInfo() {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    if (!etAlterPassword.getText().toString().equals(etAlterPasswordConfirm.getText().toString())) {
                        mUIHandler.obtainMessage(FAILURE_COMPLETE, "Passwords don't match").sendToTarget();
                    }
                    ...
                    String param = "userid=" + lastDeviceId + account + "&code="
                            + SHA1Utils.SHA256(etAlterPassword.getText().toString());
