...
public class AlterSex {
    ...
    public AlertDialog.Builder getDialog(final Activity activity) {
        ...
        builder.setTitle("Update Sex");             //设置标题
        builder.setIcon(R.mipmap.ic_launcher);//设置图标，图片id即可

        //  载入布局
        LayoutInflater inflater = activity.getLayoutInflater();
        View layout = inflater.inflate(R.layout.dialog_alert_sex, null);
        builder.setView(layout);

        //  对布局中的控件监听
        //etAlterHeight = (EditText) layout.findViewById(R.id.input_alter_height);
        mRadioMale = (RadioButton) layout.findViewById(R.id.rbtn_male);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    String param = "userid=" + lastDeviceId + account + "&sex="
                            + (mRadioMale.isChecked() ? "Male" : "Female");

                    Log.e(TAG, "" + param);
                    String rs = HttpRequestUtil.sendGet(AppConstants.URL_UPDATE_SIGN_UP_INFO, param);
                    Log.e(TAG, "" + rs);
                    mUIHandler.obtainMessage(SUCCESS_COMPLETE, "Alter Sex Successfully").sendToTarget();
                } catch (Exception ex) {
